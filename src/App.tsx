...
import { COMPLAINT_CARDS } from './data';
import { ComplaintCard, SavedChecklist } from './types';

const SANJAE_REHAB_GUIDE_URL = 'https://comwel-sanjae-rehab-guide.vercel.app';
const TOTAL_SERVICE_URL = 'https://total.comwel.or.kr/';
const COMWEL_FORM_URL = 'https://www.comwel.or.kr/comwel/info/data/papr/papr_lst.jsp';
const WORKNET_URL = 'https://www.work.go.kr/seekWantedMain.do';
...
const getOnlineLink = (card: ComplaintCard) => {
  ...
  return TOTAL_SERVICE_URL;
};

const getFormLink = () => {
  return COMWEL_FORM_URL;
};
...
const GLOSSARY_ITEMS = [
  ...
  },
  {
    ...
    spec: '(산재 치료비 지원)',
    definition:
      '업무상 사유로 부상 또는 질병이 발생하여 4일 이상 치료 및 요양이 필요한 경우, 공단의 심사를 거쳐 규정된 산재보험 범위 내의 요양비와 치료비를 지원하는 제도입니다.',
  },
  {
    term: '휴업급여',
    spec: '(치료 기간 중 휴업급여 지급)',
    definition:
      '업무상 부상 또는 질병으로 요양하느라 취업하지 못한 기간에 대하여, 생계 안정을 지원하기 위해 지급되는 산재보험 급여입니다.',
  },
  {
    term: '장해급여',
...
export default function App() {
  ...
  const findCardByCategoryAndIndex = (category: string, relativeIndex: number) => {
    const filtered = COMPLAINT_CARDS.filter(card => card.category === category);
    return filtered[relativeIndex] || null;
  };

  const activeCard = useMemo(() => {
    if (!selectedCardId) return null;
    return COMPLAINT_CARDS.find(card => card.id === selectedCardId) || null;
  }, [selectedCardId]);
          ...
          >
            ...
            <main className="mx-auto w-full max-w-4xl px-4 pb-14 flex-1">
              ...
              <section className="space-y-3.5 pt-6 text-left">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 text-center font-bold">
                  <a
                  ...
                  <a
                    href={TOTAL_SERVICE_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={() => triggerToast('고용·산재보험 토탈서비스로 이동합니다.')}
                    className="p-2.5 bg-white border border-[#DDE7F0] rounded-xl hover:border-[#0F62FE] transition-colors shadow-3xs group cursor-pointer block no-underline"
                  >
                    <span className="text-xs font-black text-[#0F294A] block group-hover:text-[#0F62FE]">
                      고용산재토탈
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={() => triggerToast('워크넷 공식 홈페이지로 이동합니다.')}
                    className="p-2.5 bg-white border border-[#DDE7F0] rounded-xl hover:border-[#0F62FE] transition-colors shadow-3xs group cursor-pointer block no-underline"
                  >
                    <span className="text-xs font-black text-[#0F294A] block group-hover:text-[#0F62FE]">
                      워크넷
                  ...
                  <a
                    ...
                    className="col-span-2 sm:col-span-2 p-2.5 bg-[#EDF5FF] border border-[#D0E2FF] rounded-xl hover:bg-white transition-colors shadow-3xs text-center block no-underline cursor-pointer"
                  >
            ...
            <main className="mx-auto w-full max-w-2xl px-4 py-6 flex-1 space-y-6">
              <section className="bg-white border border-[#DDE7F0] rounded-2xl p-5 shadow-2xs space-y-3.5 text-left">
                ...
                <div className="bg-[#F8FBFF] border border-[#DDE7F0] rounded-xl p-3 flex gap-2.5 items-start text-xs font-bold text-[#374151]">
                  ...
                  <span>
                    체크리스트 항목을 터치하여 방문 전 준비 이력을 기록할 수 있습니다. 기록은 현재 브라우저에 임시 저장됩니다.
                  </span>
                </div>
              </section>
              ...
              <section className="space-y-1.5 text-left">
                ...
                <div className="bg-white border border-[#DDE7F0] rounded-2xl p-4.5 shadow-2xs">
                  <ul className="space-y-4">
                    {activeCard.cases.map((caseText, index) => (
                      <li
                        key={index}
                        className="flex gap-2.5 items-start text-xs sm:text-sm text-[#374151] font-semibold leading-normal"
                      >
              ...
              <section className="space-y-2 text-left">
                <div className="flex items-center justify-between border-b border-[#DDE7F0] pb-1.5">
                  ...
                  {(() => {
                    ...
                    return (
                      <span className="text-sm font-serif italic text-[#6B7A90] font-medium">
                ...
                <div className="bg-white border border-[#DDE7F0] rounded-2xl p-4 shadow-2xs space-y-3">
                  <div className="space-y-2">
                    {activeCard.preChecklist.map((item, index) => {
                      const isPreChecked = !!preChecklistState[activeCard.id]?.[index];

                      return (
                        <button
                          key={index}
                          onClick={() => togglePreChecked(activeCard.id, index)}
                        ...
                        >
                          ...
                          <div
                            className={`h-[22px] w-[22px] rounded-full border flex items-center justify-center shrink-0 transition-colors ${
                              isPreChecked
              ...
              <section className="space-y-2 text-left">
                ...
                <div className="bg-white border border-[#DDE7F0] rounded-2xl p-4 shadow-2xs space-y-3">
                  <div className="space-y-2">
                    {activeCard.requiredDocs.map((documentName, index) => {
                      const isChecked = !!checklist[activeCard.id]?.[index];

                      return (
                        <button
                          key={index}
                          onClick={() => toggleDocChecked(activeCard.id, index)}
                          className={`w-full flex items-center justify-between p-3.5 text-left rounded-xl border text-xs sm:text-sm font-semibold transition-all cursor-pointer ${
                            isChecked
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="pt-2 flex flex-col gap-1.5 text-[10px] border-t border-[#DDE7F0]/70 mt-1">
                        ...
                        <div className="flex justify-between items-center text-[#6B7A90] font-bold">
                          <span>지참 서류 완성도</span>
                          <span className="text-[#15803D] font-serif">
                            {percentage}% ({checkedCount}/{total})
                          </span>
                        </div>
                      </div>
                ...
                <section className="space-y-1.5 text-left">
                  ...
                  <div className="bg-white border border-[#DDE7F0] rounded-2xl p-4.5 shadow-2xs">
                    <ul className="space-y-2.5">
                      {activeCard.helpfulDocs.map((documentName, index) => (
                        <li
                          key={index}
                          className="flex gap-2.5 items-start text-xs sm:text-sm text-[#374151] font-medium leading-normal"
                        >
              ...
              <section className="space-y-2 text-left">
                ...
                <div className="bg-white border border-[#DDE7F0] rounded-2xl p-5 shadow-2xs space-y-5 relative overflow-hidden">
                  ...
                  {activeCard.procedures.map((procedure, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="flex gap-4 items-start text-xs sm:text-sm leading-relaxed relative z-10 font-bold"
                      >
                        <div
                          className={`h-[24px] w-[24px] rounded-full text-[9px] font-black flex items-center justify-center shrink-0 mt-0.5 select-none border transition-all ${
                            isDone
                        ...
                        <div className="flex-1 space-y-0.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`inline-block text-[8px] sm:text-[9px] font-black px-1.5 py-0.5 rounded ${
                                isDone
                          ...
                          <p
                            className={`${
                              isDone
                                ? 'text-[#6B7A90] font-medium'
                                : isCurrent
                                ? 'text-[#0F294A] font-extrabold'
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-2.5 font-bold">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={() => triggerToast('[안내] 고용·산재보험 토탈서비스로 이동합니다.')}
                      className="inline-flex flex-col items-center justify-center gap-1.5 rounded-2xl bg-white border border-[#DDE7F0] px-3 py-3.5 text-xs text-[#0F294A] hover:bg-[#F8FBFF] active:scale-97 transition-all shadow-3xs cursor-pointer no-underline text-center"
                    >
      ...
      <footer className="w-full bg-[#0F294A] text-white pt-12 pb-10 px-4 sm:px-6 relative overflow-hidden shrink-0 mt-auto border-t border-[#0B1F38]">
        ...
        <div className="mx-auto max-w-4xl space-y-6 text-center relative z-10">
          ...
          <p className="text-[10px] sm:text-[11.5px] text-white/75 leading-relaxed max-w-xl mx-auto font-medium">
            본 서비스는 근로복지공단 지사를 방문하기 전 필요 서류를 자가 확인하도록 돕는{' '}
            <span className="text-[#78A9FF] font-black">비공식 길잡이 도구</span>입니다. 정부, 고용노동부, 근로복지공단 등의 공식 국가 기관 및 공공부처를 대변하거나 대리하지 않습니다.
          </p>
